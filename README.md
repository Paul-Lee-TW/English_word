...
- **新しい単語**：先選等級（Lv1~Lv6 或全部），可切換「順番どおり／ランダム」出題順序，每次學習 5~30 個新單字（翻卡＋自動發音）；ランダム設定同樣套用於熟語學習。卡片支援滑動手勢：左滑＝もう知ってる、右滑＝学習した（翻面前後皆可）
- **熟語・連語**：內建 235 個國中重要熟語（`data/phrases.json`，get up、be interested in、look forward to…），獨立學習入口與列表「熟語」分頁，複習測驗的干擾選項取自其他熟語；含空格的條目不會出現在拼寫練習
